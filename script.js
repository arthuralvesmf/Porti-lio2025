...
function initializeScrollEffects() {
    ...
    const animateElements = document.querySelectorAll('.stat-card, .experience-item, .skill-category, .project-card, .contact-item');
...
function animateCounter(card) {
    ...
    const timer = setInterval(() => {
        ...
        numberElement.textContent = isPercentage ? `${displayValue}%` : 
                                   finalNumber.includes('+') ? `${displayValue}+` : displayValue;
    }, 50);
}
